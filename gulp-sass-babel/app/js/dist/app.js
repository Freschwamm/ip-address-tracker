...
document.querySelector('button').addEventListener('click', function (e) {
  var ipInput = document.querySelector('.ipSearch').value;
  var ipformat = /^(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$/;

  if (ipInput.match(ipformat)) {
    fetch('https://geo.ipify.org/api/v1?apiKey=at_zDED4T1siN02XXzyDJcKePaXQ127v&ipAddress=' + ipInput).then(function (data) {
      return data.json();
    }).then(function (data) {
      if (mymap !== undefined) {
        mymap.remove();
